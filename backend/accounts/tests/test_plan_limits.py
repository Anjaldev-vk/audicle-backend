...
def free_user(db):
    ...
    # Signal should have created Free subscription, but let's ensure it's there
...
def test_free_org_cannot_exceed_member_limit(auth_client, free_user, db):
    org = Organisation.objects.create(
        name='Free Org', slug='free-org'
    )
    # Ensure org has Free subscription
    free_plan = Plan.objects.get(name='Free')
    Subscription.objects.update_or_create(
        organisation=org,
        defaults={'plan': free_plan, 'status': 'active'}
    ...
    # Add 1 more member to hit limit of 2
...
def test_pro_user_can_dispatch_bot(pro_client, pro_user, db):
...
def test_pro_user_can_create_chat_session(pro_client):
    res = pro_client.post(
...
def test_free_user_cannot_exceed_meeting_limit(auth_client, free_user):
    with patch('utils.plan_limits._get_monthly_meeting_count', return_value=5):
        res = auth_client.post(
            '/api/v1/meetings/',
            {
