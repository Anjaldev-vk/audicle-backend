...
for uid in users:
    ...
    print(f"User {uid} Count: {response['Count']}")
    for item in response.get('Items', []):
        if item.get('event_type') == 'meeting_completed':
             print(f"  - Completed Meeting: {item.get('metadata', {}).get('meeting_id')} | Duration: {item.get('metadata', {}).get('duration_seconds')}")
