...
class TestBotDispatchView:
    ...
    def test_dispatch_success(self, mock_send, org_admin_client, zoom_meeting):
        """Bot dispatch sends to bot_tasks topic and sets status to bot_joining."""
        url = self._url(zoom_meeting.id)
        resp = org_admin_client.post(url)
    ...
    @patch("utils.kafka_producer.send_bot_task", return_value=True)
    def test_dispatch_rejects_upload_platform(self, mock_send, org_admin_client, upload_meeting):
    ...
    @patch("utils.kafka_producer.send_bot_task", return_value=True)
    def test_dispatch_rejects_non_scheduled_meeting(self, mock_send, org_admin_client, zoom_meeting):
    ...
    @patch("utils.kafka_producer.send_bot_task", return_value=True)
    def test_dispatch_allowed_on_failed_meeting(self, mock_send, org_admin_client, zoom_meeting):
    ...
    def test_dispatch_requires_authentication(self, api_client, zoom_meeting):
        """Unauthenticated requests are rejected."""
        url = self._url(zoom_meeting.id)
        resp = api_client.post(url)
        assert resp.status_code == status.HTTP_401_UNAUTHORIZED

    def test_dispatch_meeting_not_found(self, org_admin_client):
    ...
    def test_dispatch_kafka_failure_returns_503(self, mock_send, org_admin_client, zoom_meeting):
...
class TestBotStatusView:
    ...
    def test_missing_secret_rejected(self, zoom_meeting):
    ...
    def test_wrong_secret_rejected(self, zoom_meeting, internal_secret):
    ...
    def test_bot_joining_updates_status(self, zoom_meeting, internal_secret):
    ...
    def test_recording_updates_status_and_started_at(self, zoom_meeting, internal_secret):
    ...
    def test_processing_stores_s3_key_and_triggers_transcription(
        self, mock_transcription, zoom_meeting, internal_secret
    ):
    ...
    def test_failed_status_updates_meeting(self, zoom_meeting, internal_secret):
    ...
    def test_invalid_meeting_id_returns_404(self, internal_secret):
...
class TestAutoDispatchBotsTask:
    ...
    @patch("utils.kafka_producer.send_bot_task", return_value=True)
    def test_dispatches_meeting_in_window(self, mock_send, org_admin, organisation):
    ...
    @patch("utils.kafka_producer.send_bot_task", return_value=True)
    def test_skips_upload_meetings(self, mock_send, org_admin, organisation):
    ...
    @patch("utils.kafka_producer.send_bot_task", return_value=True)
    def test_skips_archived_meetings(self, mock_send, org_admin, organisation):
